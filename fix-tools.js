...
// Helper: replace the <script>...</script> block in an HTML file
function replaceScript(file, newScript) {
  let html = fs.readFileSync(file, 'utf8');
  // Replace everything from <script> to </script> at the end
  const idx = html.lastIndexOf('<script>');
  if (idx === -1) {
    console.log('WARN: no <script> in', file);
    return;
  }
  const endIdx = html.lastIndexOf('</script>');
  const newHtml = html.substring(0, idx + 8) + '\n' + newScript + '\n' + html.substring(endIdx);
...
const lottery = `
const poolIn = document.createElement('textarea'); poolIn.className='tool-textarea'; poolIn.placeholder='输入参与名单，每行一个...';
...
c.innerHTML = '<div class="tool-label">输入内容</div><textarea class="tool-textarea" id="tin" placeholder="输入文本..."></textarea><div class="tool-row" style="margin-top:8px;"><button class="tool-btn" onclick="encode()">URL 编码</button><button class="tool-btn" onclick="decode()">URL 解码</button><button class="tool-btn secondary" onclick="copyR()">复制结果</button></div><div class="tool-result" id="r" style="display:none;"></div>';
function encode(){const v=document.getElementById('tin').value; const r=document.getElementById('r'); r.style.display='block'; r.innerText=encodeURIComponent(v);}
function decode(){const v=document.getElementById('tin').value; const r=document.getElementById('r'); try{r.style.display='block'; r.innerText=decodeURIComponent(v);}catch(e){r.innerText='解码失败：'+e.message;}}
...
function format(ms){const h=Math.floor(ms/3600000);const m=Math.floor((ms%3600000)/60000);const s=Math.floor((ms%60000)/1000);const f=ms%1000;return String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')+':'+String(s).padStart(2,'0')+'.'+String(f).padStart(3,'0');}
function tick(){elapsed=Date.now()-startTime;document.getElementById('r').innerText=format(elapsed);}
...
function render(){const v=document.getElementById('tin').value;let h=v;h=h.replace(/^### (.*$)/gim,'<h3>$1</h3>');h=h.replace(/^## (.*$)/gim,'<h2>$1</h2>');h=h.replace(/^# (.*$)/gim,'<h1>$1</h1>');h=h.replace(/\\*\\*(.*?)\\*\\*/g,'<strong>$1</strong>');h=h.replace(/\\*(.*?)\\*/g,'<em>$1</em>');h=h.replace(/`([^`]+)`/g,'<code style="background:#f0f0f0;padding:2px 4px;border-radius:3px;">$1</code>');h=h.replace(/\\n/g,'<br>');document.getElementById('preview').innerHTML=h;}
...
function toCn(){const v=document.getElementById('tin').value;const r=document.getElementById('r');if(!v){alert('请输入数字');return;}r.style.display='block';r.innerText='中文大写: '+numToCn(+v);}
function numToCn(n){if(n===0)return '零';const s=String(n);let res='';let groupIdx=0;for(let i=s.length;i>0;i-=4){const group=s.slice(Math.max(0,i-4),i);let gRes='';let zero=false;for(let j=0;j<group.length;j++){const d=+group[j];const pos=group.length-1-j;if(d===0){if(!zero&&gRes)gRes+=cnNums[0];zero=true;}else{gRes+=cnNums[d]+cnUnits[pos];zero=false;}}if(gRes)res=gRes+cnBigUnits[groupIdx]+res;groupIdx++;}return res.replace(/零+/g,'零').replace(/零$/,'');}
function copyR(){navigator.clipboard.writeText(document.getElementById('r').innerText).then(()=>alert('已复制'));}
`;
...
function conv(){const v=document.getElementById('tin').value;const r=document.getElementById('r');r.style.display='block';const map={'\u4e00':'yi','\u4e8c':'er','\u4e09':'san','\u56db':'si','\u4e94':'wu','\u516d':'liu','\u4e03':'qi','\u516b':'ba','\u4e5d':'jiu','\u5341':'shi'};let h='';for(const ch of v){const code=ch.charCodeAt(0);if(code>=0x4e00&&code<=0x9fa5){h+=map[ch]||'('+(ch)+')';}else{h+=ch;}}r.innerText=h;}
function copyR(){navigator.clipboard.writeText(document.getElementById('r').innerText).then(()=>alert('已复制'));}
`;
...
function toJson(){const v=document.getElementById('tin').value;const r=document.getElementById('r');r.style.display='block';const lines=v.split('\\n');const obj={};let current=obj;const stack=[obj];for(const line of lines){const m=line.match(/^(\\s*)(\\w+):\\s*(.*)$/);if(m){const indent=m[1].length;const key=m[2];const val=m[3].trim();if(!val){current[key]={};current=current[key];stack.push(current);}else{current[key]=isNaN(+val)?val:+val;}}}r.innerText=JSON.stringify(obj,null,2);}
...
function fmt(){const v=document.getElementById('tin').value;const r=document.getElementById('r');r.style.display='block';r.innerText=v.replace(/>\\s*</g,'>\\n<').replace(/<\\/(\\w+)>/g,'</$1>').split('\\n').map((line,i)=>{const indent=(line.match(/<\\//g)||[]).length;const open=(line.match(/<[^/\\s][^>]*>/g)||[]).length;const close=(line.match(/<\\//g)||[]).length;return '  '.repeat(Math.max(0,i>0?open-close+indent:0))+line.trim();}).join('\\n');}
function copyR(){navigator.clipboard.writeText(document.getElementById('r').innerText).then(()=>alert('已复制'));}
`;
...
const map = {
  ...
  'lipsum-generator': lipsumGenerator,
  'chinese-number-game': chineseNumberGame,
  'number-to-chinese': numberToChinese,
...
let fixed = 0;
for (const [url, script] of Object.entries(map)) {
  const file = url + '.html';
  if (!fs.existsSync(file)) {
    console.log('MISSING file:', file);
    continue;
  }
  replaceScript(file, script);
  fixed++;
}

console.log('Fixed', fixed, 'tool pages!');
